...
class StockMasterUpdatesRepository(BaseRepository[StockMasterUpdates]):
    ...
    def __init__(self, session: AsyncSession):
        """初期化。セッションを受け取り `StockMasterUpdates` モデルをセットします."""
    ...
    async def create_summary(self, data: Dict) -> StockMasterUpdates:
        ...
        try:
            ...
            logger.exception("Failed to create stock_master_update summary: %s", e)
            raise MasterDataError(message=f"Failed to create summary: {e}") from e
    ...
    async def update_status(
        ...
        try:
            ...
            raise MasterDataError(message=f"Failed to update status: {e}") from e
    ...
    async def delete_by_reset(self) -> int:
        ...
        try:
            ...
            logger.info("Deleted %d stock_master_updates records", deleted_count)
            return deleted_count
        except Exception as e:
            logger.exception("Failed to delete stock_master_updates records: %s", e)
            raise MasterDataError(message=f"Failed to delete records: {e}") from e
    ...
    async def get_by_id(self, record_id: int) -> Optional[StockMasterUpdates]:
        ...
        try:
            ...
            logger.exception("Failed to get stock_master_update id=%s: %s", record_id, e)
