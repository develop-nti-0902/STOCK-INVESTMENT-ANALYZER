"""EDINET 損益・キャッシュフロー用 Repository 実装.
...
class EdinetProfitAndLossRepository(BaseRepository[EdinetProfitAndLoss]):
    """`edinet_profit_and_loss` テーブル向けの Repository 実装.
    ...
    def __init__(self, session: AsyncSession):
        """初期化。セッションを受け取り `EdinetProfitAndLoss` モデルをセットします."""
    ...
    async def find_latest_by_sec_code(self, sec_code: str) -> Optional[EdinetProfitAndLoss]:
        """指定の `sec_code` に対する最新の損益・キャッシュフローレコードを返します."""
        stmt = (
            select(self.model)
    ...
    async def upsert(self, data: dict) -> EdinetProfitAndLoss:
        ...
        try:
            ...
            logger.exception("edinet profit and loss upsert failed: %s", e)
            raise
