...
class EdinetProfitAndLossRepository(BaseRepository[EdinetProfitAndLoss]):
    """EdinetProfitAndLoss モデル用のリポジトリ。"""
    ...
    async def save_batch(self, data_list: list[dict]) -> list[EdinetProfitAndLoss]:
        ...
        Returns:
            保存後の EdinetProfitAndLoss インスタンスのリスト
        ...
        table = self.model.__table__
        all_results: list[EdinetProfitAndLoss] = []
        ...
        for i in range(0, len(data_list), _UPSERT_CHUNK_SIZE):
            ...
            try:
                ...
                logger.exception("save_batch_upsert failed for edinet_profit_and_loss")
                raise
