...
class EdinetBalanceSheet(SerialPKMixin, TimestampMixin, Base):
    ...
    カラム定義やインデックスは `docs/architecture/edinet_balance_sheet_design.md`
    に従っています。
    """
    ...
    short_term_loans: Mapped[Optional[float]] = mapped_column(
        Numeric(20, 2), nullable=True
    )
    long_term_loans: Mapped[Optional[float]] = mapped_column(
        Numeric(20, 2), nullable=True
    )
    ...
    shareholders_equity: Mapped[Optional[float]] = mapped_column(
        Numeric(20, 2), nullable=True
    )
    ...
    equity_to_asset_ratio: Mapped[Optional[float]] = mapped_column(
        Numeric(5, 2), nullable=True
    )

    candidate_contexts: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )
    candidate_keys: Mapped[Optional[str]] = mapped_column(
        String(50), nullable=True
    )
    is_consolidated: Mapped[Optional[bool]] = mapped_column(
    ...
    __table_args__ = (
        ...
        Index(
            "idx_edinet_balance_sec_code_period", "sec_code", "period_end_date"
        ),
        UniqueConstraint(
