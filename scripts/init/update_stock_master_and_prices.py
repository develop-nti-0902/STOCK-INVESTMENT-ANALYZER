...
async def _run(timeframe: str = "1d", batch_size: int = 500) -> None:
    ...
    async with session_maker() as session:  # type: AsyncSession
        ...
        # 全銘柄はバッチランナーでチャンク処理する
        batch_runner = StockPriceBatchRunner(
            batch_service=batch_service,
            stock_price_service=sp_service,
            stock_master_service=stock_master_service,
        )

        logger.info(
            "Starting batch run for all JPX symbols, timeframe=%s, batch_size=%s",
            timeframe,
            batch_size,
        )

        summary = await batch_runner.execute_jpx_all_for_timeframe(
            timeframe=timeframe, batch_size=batch_size
        )
