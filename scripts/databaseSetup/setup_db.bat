...
if exist "%REPO_ROOT%.env" (
    ...
    for /f "usebackq tokens=1* delims==" %%A in ("%REPO_ROOT%.env") do (
        if not "%%A"=="" (
            ...
            if /I "%%~A"=="NEW_DB_PASSWORD" if not defined NEW_DB_PASSWORD set "NEW_DB_PASSWORD=%%~B"
...
REM setup_db.bat PGHOST PGPORT PGUSER PGPASSWORD NEW_DB NEW_DB_USER NEW_DB_PASSWORD
...
if not "%ARG7%"=="" set NEW_DB_PASSWORD=%ARG7%
...
if not defined NEW_DB_PASSWORD (
...
psql -h %PGHOST% -p %PGPORT% -U %PGUSER% -c "DO $$ BEGIN IF NOT EXISTS (SELECT FROM pg_catalog.pg_user WHERE usename = '%NEW_DB_USER%') THEN CREATE USER %NEW_DB_USER% WITH PASSWORD '%NEW_DB_PASSWORD%'; END IF; END$$;" 2>NUL
...
if not "%DB_EXISTS%"=="1" (
    if not "%SKIP_TABLESPACE%"=="" (
        ...
        psql -U %PGUSER% -h %PGHOST% -c "CREATE DATABASE !NEW_DB! WITH OWNER = %PGUSER% ENCODING = 'UTF8' LC_COLLATE = 'C' LC_CTYPE = 'C' TEMPLATE = template0 CONNECTION LIMIT = -1;" 2>"%TEMP%\db_create_err.txt"
    ) else (
        REM Create database with custom tablespace
        psql -U %PGUSER% -h %PGHOST% -c "CREATE DATABASE !NEW_DB! WITH OWNER = %PGUSER% ENCODING = 'UTF8' LC_COLLATE = 'C' LC_CTYPE = 'C' TABLESPACE = stock_data_space TEMPLATE = template0 CONNECTION LIMIT = -1;" 2>"%TEMP%\db_create_err.txt"
    )
    if errorlevel 1 (
...
psql -h %PGHOST% -p %PGPORT% -U %PGUSER% -d %NEW_DB% -c "GRANT ALL PRIVILEGES ON DATABASE %NEW_DB% TO %NEW_DB_USER%;" 2>NUL || echo [WARN] Could not grant privileges
