...
from app.repositories.stock_data_repository import StockData1hRepository
...
async def test_fetch_and_save_1h_stock_data(monkeypatch):
    """
    統合テスト: yfinanceから1h株価データをフェッチし、Stocks1hテーブルへ保存するまでの
    一連の流れを検証する。
    """
    ...
    async with session_maker() as session:
        repo = StockData1hRepository(session=session)
