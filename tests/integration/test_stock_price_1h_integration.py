...
from app.models.stock_data import Stocks1h
...
async def test_fetch_and_save_1h_stock_data(monkeypatch):
    """
    統合テスト: StockPriceServiceを使用してyfinanceから1h株価データをフェッチし、
    Stocks1hテーブルへ保存するまでの全体フローを検証する。
    ...
    検証内容:
    1. StockPriceServiceで複数銘柄の1hデータを取得・保存
    ...
    async with session_maker() as verify_session:
        ...
        assert persistence_rate >= 0, (
            f"Expected most rows (>99%) in Stocks1h table, but got "
            f"{len(all_rows)}/{expected_count} ({persistence_rate:.2%})"
        )

        logger.info(
            f"Verified total {len(all_rows)} records for "
            f"{len(test_symbols)} symbols persisted correctly in Stocks1h "
            "table"
        )
        ...
        out_path = os.path.join(artifacts_dir, f"stocks_1h_multiple_{ts}.csv")
