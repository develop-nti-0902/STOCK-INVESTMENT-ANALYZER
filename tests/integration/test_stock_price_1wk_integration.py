...
from app.repositories.stock_data_repository import StockData1wkRepository
...
async def test_fetch_and_save_1wk_stock_data(monkeypatch):
    """
    統合テスト: yfinanceから1wk株価データをフェッチし、Stocks1wkテーブルへ保存するまでの
    一連の流れを検証する。
    """
    ...
    async with session_maker() as session:
        repo = StockData1wkRepository(session=session)
