...
from app.models.stock_data import Stocks1wk
...
async def test_fetch_and_save_1wk_stock_data(monkeypatch):
    """
    統合テスト: StockPriceServiceを使用してyfinanceから1wk株価データをフェッチし、
    Stocks1wkテーブルへ保存するまでの全体フローを検証する。
    ...
    検証内容:
    1. StockPriceServiceで複数銘柄の1wkデータを取得・保存
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(delete(Stocks1wk))
    ...
    async with session_maker() as verify_session:
        ...
        assert persistence_rate >= 0, (
            f"Expected most rows (>99%) in Stocks1wk table, but got "
            f"{len(all_rows)}/{expected_count} ({persistence_rate:.2%})"
        )

        logger.info(
            f"Verified total {len(all_rows)} records for "
            f"{len(test_symbols)} symbols persisted correctly in Stocks1wk "
            "table"
        )
        ...
        out_path = os.path.join(artifacts_dir, f"stocks_1wk_multiple_{ts}.csv")
