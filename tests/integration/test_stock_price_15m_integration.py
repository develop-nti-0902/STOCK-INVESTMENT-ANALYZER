...
from app.repositories.stock_data_repository import StockData15mRepository
...
async def test_fetch_and_save_15m_stock_data(monkeypatch):
    """
    統合テスト: yfinanceから15m株価データをフェッチし、Stocks15mテーブルへ保存するまでの
    一連の流れを検証する。
    """
    ...
    async with session_maker() as session:
        repo = StockData15mRepository(session=session)
