...
from app.repositories.stock_data_repository import StockData30mRepository
...
async def test_fetch_and_save_30m_stock_data(monkeypatch):
    """
    統合テスト: yfinanceから30m株価データをフェッチし、Stocks30mテーブルへ保存するまでの
    一連の流れを検証する。
    """
    ...
    async with session_maker() as session:
        repo = StockData30mRepository(session=session)
