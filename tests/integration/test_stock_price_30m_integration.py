...
from app.models.stock_data import Stocks30m
...
async def test_fetch_and_save_30m_stock_data(monkeypatch):
    """
    統合テスト: StockPriceServiceを使用してyfinanceから30m株価データをフェッチし、
    Stocks30mテーブルへ保存するまでの全体フローを検証する。
    ...
    検証内容:
    1. StockPriceServiceで複数銘柄の30mデータを取得・保存
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(delete(Stocks30m))
    ...
    async with session_maker() as verify_session:
        ...
        assert persistence_rate >= 0, (
            f"Expected most rows (>99%) in Stocks30m table, but got "
            f"{len(all_rows)}/{expected_count} ({persistence_rate:.2%})"
        )

        logger.info(
            f"Verified total {len(all_rows)} records for "
            f"{len(test_symbols)} symbols persisted correctly in Stocks30m "
            "table"
        )
        ...
        out_path = os.path.join(artifacts_dir, f"stocks_30m_multiple_{ts}.csv")
