...
async def test_fetch_and_save_1mo_stock_data(monkeypatch):
    """
    統合テスト: StockPriceServiceを使用してyfinanceから1mo株価データをフェッチし、
    Stocks1moテーブルへ保存するまでの全体フローを検証する。
    ...
    検証内容:
    1. StockPriceServiceで複数銘柄の1moデータを取得・保存
    ...
    async with session_maker() as verify_session:
        ...
        logger.info(
            ...
            f"{len(test_symbols)} symbols persisted correctly in Stocks1mo "
            "table"
        )
