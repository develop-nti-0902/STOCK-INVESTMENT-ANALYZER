...
from app.models.stock_data import Stocks1m
from app.repositories.stock_data_repository import StockData1mRepository
...
async def test_fetch_and_save_1m_stock_data(monkeypatch):
    """
    統合テスト: yfinanceから1m株価データをフェッチし、Stocks1mテーブルへ保存するまでの
    一連の流れを検証する。
    ...
    検証内容:
    1. StockPriceFetcherで特定銘柄の1mデータを取得
    2. StockPriceSaverを使用してStocks1mテーブルへ保存
    ...
    # Act（実行）: StockData1mRepositoryで複数銘柄のデータを保存
    async with session_maker() as session:
        # StockData1mRepositoryを直接使用して保存
        repo = StockData1mRepository(session=session)

        # 全銘柄のStockDataリストをDB形式の辞書リストに変換
        # Stocks1mモデルは "symbol" と "timestamp" カラムを持っている
        all_db_records = []
    ...
    async with session_maker() as verify_session:
        ...
        assert persistence_rate > 0.99, (
            f"Expected most rows (>99%) in Stocks1m table, but got "
            f"{len(all_rows)}/{expected_count} ({persistence_rate:.2%})"
        )

        logger.info(
            f"Verified total {len(all_rows)} records for "
            f"{len(test_symbols)} symbols persisted correctly in Stocks1m "
            "table"
        )
        ...
        out_path = os.path.join(artifacts_dir, f"stocks_1m_multiple_{ts}.csv")
