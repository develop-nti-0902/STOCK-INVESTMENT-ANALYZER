...
def test_tablename_and_columns_exist():
    # テーブル名が仕様通りであること
    assert (
        getattr(EdinetBalanceSheet, "__tablename__") == "edinet_balance_sheets"
    )
...
def test_table_args_contains_unique_constraint():
    ...
    uc_found = any(
        isinstance(t, object)
        and getattr(t, "name", None) == "uq_edinet_sec_period"
        for t in table_args
    )
    assert (
        uc_found
    ), "UniqueConstraint 'uq_edinet_sec_period' not found in __table_args__"
