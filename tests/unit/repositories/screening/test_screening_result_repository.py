...
async def test_upsert_inserts_and_returns_model(repository, mock_session):
    ...
    repository.find_latest_by_sec_code = AsyncMock(return_value=expected)
    ...
    repository.find_latest_by_sec_code.assert_awaited_once_with("7203")
...
@pytest.mark.asyncio
async def test_find_by_sec_code_returns_history(repository, mock_session):
    ...
    result = await repository.find_by_sec_code("7203")
...
async def test_list_filters_and_pagination(repository, mock_session):
    ...
    mock_result = MagicMock()
    mock_scalars = MagicMock()
    record = make_model(sec_code="6758", total_score=88)
    mock_scalars.all.return_value = [record]
    mock_result.scalars.return_value = mock_scalars
    mock_session.execute.return_value = mock_result

    result = await repository.list(
        skip=1, limit=5, min_score=70, status="active", sec_codes=["6758"]
    )
