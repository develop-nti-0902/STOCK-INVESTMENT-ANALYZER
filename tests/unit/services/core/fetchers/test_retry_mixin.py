...
class TestRetryMixin:
    ...
    def retry_mixin(self) -> RetryMixin:
        ...
        with patch(
            "app.services.core.fetchers.retry_mixin.get_settings"
        ) as mock_settings:
    ...
    def test_is_retryable_error_connection_error(
        ...
        # Act & Assert - 実行と検証
        assert retry_mixin._is_retryable_error(
            ConnectionError("Connection failed")
        )

    def test_is_retryable_error_timeout_error(
        self, retry_mixin: RetryMixin
    ) -> None:
    ...
    def test_is_retryable_error_os_error(
        self, retry_mixin: RetryMixin
    ) -> None:
    ...
    def test_is_retryable_error_value_error(
        self, retry_mixin: RetryMixin
    ) -> None:
    ...
    def test_is_retryable_error_with_message(
        self, retry_mixin: RetryMixin
    ) -> None:
        """エラーメッセージによる判定テスト"""
        # Arrange - 準備
        ...
        assert not retry_mixin._is_retryable_error(
    ...
    def test_update_retry_config_partial(
        self, retry_mixin: RetryMixin
    ) -> None:
        """部分的なリトライ設定更新テスト"""
        # Arrange - 準備
