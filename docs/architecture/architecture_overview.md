...
- [システムアーキテクチャ概要](#システムアーキテクチャ概要)
  ...
  - [1. プロジェクト概要](#1-プロジェクト概要)
    ...
    - [設計理念](#設計理念)
  - [2. 主要機能一覧](#2-主要機能一覧)
    - [1. 日本株データのDB格納(データプラットフォーム)](#1-日本株データのdb格納データプラットフォーム)
    - [2. ダッシュボード](#2-ダッシュボード)
    - [3. 銘柄検索と詳細表示](#3-銘柄検索と詳細表示)
    - [4. 分析ツール(スクリーニング)](#4-分析ツールスクリーニング)
    - [5. バックテスト(簡易)](#5-バックテスト簡易)
    - [6. ユーザー設定と認証](#6-ユーザー設定と認証)
    - [共通機能(監視・管理)](#共通機能監視管理)
  - [3. システム全体像](#3-システム全体像)
    - [アーキテクチャ図(4層構造)](#アーキテクチャ図4層構造)
    - [データフロー(非同期処理)](#データフロー非同期処理)
  - [4. アーキテクチャ構成(4層構造)](#4-アーキテクチャ構成4層構造)
    - [レイヤー構成](#レイヤー構成)
    - [レイヤー別責任と配置](#レイヤー別責任と配置)
    - [共通モジュール](#共通モジュール)
    - [4層構造の利点](#4層構造の利点)
      - [1. 責任の明確な分離](#1-責任の明確な分離)
      - [2. 並行開発の実現](#2-並行開発の実現)
      - [3. 契約駆動開発(Contract-First Development with FastAPI)](#3-契約駆動開発contract-first-development-with-fastapi)
      - [4. Repository Patternの採用(非同期対応)](#4-repository-patternの採用非同期対応)
  - [5. 技術スタック](#5-技術スタック)
    - [FastAPI選定理由](#fastapi選定理由)
    - [バックエンド](#バックエンド)
    - [フロントエンド](#フロントエンド)
      - [チャートライブラリ選定理由: Lightweight Charts](#チャートライブラリ選定理由-lightweight-charts)
      - [APIクライアント選定理由: Fetch API](#apiクライアント選定理由-fetch-api)
    - [データベース](#データベース)
      - [データベーステーブル設計](#データベーステーブル設計)
    - [開発・テスト](#開発テスト)
  - [6. ディレクトリ構成(4層構造対応)](#6-ディレクトリ構成4層構造対応)
    - [アプリケーション全体構成](#アプリケーション全体構成)
    - [実装優先順位](#実装優先順位)
      - [フェーズ1: コア機能(データプラットフォーム)](#フェーズ1-コア機能データプラットフォーム)
      - [フェーズ2: 基本UI](#フェーズ2-基本ui)
      - [フェーズ3: 財務・分析機能](#フェーズ3-財務分析機能)
      - [フェーズ4: 高度な機能](#フェーズ4-高度な機能)
    - [FastAPI固有の設計要素](#fastapi固有の設計要素)
      - [Pydanticスキーマの役割](#pydanticスキーマの役割)
      - [非同期処理の実装方針](#非同期処理の実装方針)
      - [依存性注入の活用](#依存性注入の活用)
  - [関連ドキュメント](#関連ドキュメント)
...
| 機能                           | できること                                                                                                                                                                           | エンドポイント/技術                                                                                           | 実装レイヤー                            | 実装状況   |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- | --------------------------------------- | ---------- |
| **JPX全銘柄取得**              | 4,000銘柄以上を自動取得<br>バッチ実行履歴の記録                                                                                                                                      | `POST /api/v1/batch/stock-data/jpx-all`<br>`GET /api/v1/batch/status/{job_id}`<br>`GET /api/v1/batch/history` | API層<br>サービス層                     | ✅ 実装済み |
| **マルチタイムフレーム管理**   | 8種類の時間軸データを自動振り分け<br>(1分足、5分足、15分足、30分足、1時間足、1日足、1週足、1月足)<br>重複チェックとUPSERT操作                                                        | サービス層(StockPriceSaver)                                                                                   | サービス層<br>データアクセス層          | ✅ 実装済み |
| **ファンダメンタルデータ取得** | EPS、BPS、売上、営業利益、純利益、ROE、自己資本比率等の財務指標取得<br>Yahoo Finance APIから財務データを取得し、fundamental_dataテーブルに格納<br>年次・四半期データの両方をサポート | `POST /api/fundamental/fetch`<br>`GET /api/fundamental/{symbol}`                                              | API層<br>サービス層<br>データアクセス層 | 🚧 未実装   |

### 2. ダッシュボード

| 機能                         | できること                                                                                                                                               | エンドポイント/画面                                                                                           | 実装レイヤー                                                    | 実装状況                           |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ---------------------------------- |
| **ポートフォリオ概況表示**   | ポートフォリオ評価額、保有銘柄一覧、損益情報の確認<br>リアルタイム株価との連動で現在評価額を計算<br>銘柄別保有数量・平均取得単価・現在価格・損益率を表示 | `GET /api/portfolio/summary`<br>`GET /api/portfolio/holdings`                                                 | API層<br>サービス層<br>データアクセス層<br>プレゼンテーション層 | 🚧 未実装                           |
| **主要インデックス表示**     | 日経平均、TOPIX、マザーズ指数等の時系列データ表示<br>日次・週次・月次の推移チャート<br>前日比・騰落率の表示                                              | `GET /api/indices/list`<br>`GET /api/indices/{index_code}/history`                                            | API層<br>サービス層<br>データアクセス層                         | 🚧 未実装                           |
| **データ取得ジョブ管理**     | 手動トリガボタン、ジョブステータス表示<br>実行履歴の確認、進捗状況のリアルタイム表示                                                                     | `POST /api/v1/batch/stock-data/jpx-all`<br>`GET /api/v1/batch/status/{job_id}`<br>`GET /api/v1/batch/history` | API層<br>プレゼンテーション層<br>フロントエンド                 | ✅ 実装済み (API層のみ、UIは未実装) |
| **ウィジェットカスタマイズ** | ダッシュボードの表示項目カスタマイズ<br>ウィジェットの表示/非表示、配置順序の変更<br>ユーザー設定の永続化                                                | `GET /api/user/dashboard-settings`<br>`PUT /api/user/dashboard-settings`                                      | API層<br>サービス層<br>データアクセス層                         | 🚧 未実装                           |

### 3. 銘柄検索と詳細表示

| 機能                     | できること                                                                                                                                                       | エンドポイント                                                                                                   | 実装レイヤー                                                                      | 実装状況   |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | ---------- |
| **銘柄マスタ検索**       | 銘柄コード/名称で検索<br>JPX全上場銘柄の基本情報管理                                                                                                             | `POST /api/v1/stock-master/refresh`<br>`GET /api/v1/stock-master/symbols`<br>`GET /api/v1/stock-master/{symbol}` | API層<br>サービス層<br>データアクセス層                                           | ✅ 実装済み |
| **株価データ参照**       | 時系列ごとの価格情報(OHLCV)の取得<br>ページネーション対応                                                                                                        | `GET /api/v1/stock-price/{symbol}`                                                                               | API層<br>データアクセス層                                                         | ✅ 実装済み |
| **チャート表示**         | 複数期間の株価チャート表示<br>ローソク足、移動平均線(SMA/EMA)、出来高の表示<br>期間選択(1日、5日、1ヶ月、3ヶ月、1年、5年、全期間)<br>時間軸切り替え(1分足〜月足) | `GET /api/stocks/{symbol}/chart`                                                                                 | API層<br>サービス層<br>プレゼンテーション層<br>フロントエンド: Lightweight Charts | 🚧 未実装   |
| **ファンダメンタル表示** | 財務指標の表示<br>EPS、PER、PBR、ROE、配当利回り等<br>年次・四半期推移グラフ<br>業界平均との比較                                                                 | `GET /api/stocks/{symbol}/fundamental`                                                                           | API層<br>サービス層<br>データアクセス層                                           | 🚧 未実装   |
| **銘柄比較機能**         | 複数銘柄(最大5銘柄)の並列比較<br>株価推移の重ね合わせチャート<br>財務指標の並列表示<br>騰落率の比較                                                              | `POST /api/stocks/compare`                                                                                       | API層<br>サービス層<br>プレゼンテーション層                                       | 🚧 未実装   |

### 4. 分析ツール(スクリーニング)

| 機能                       | できること                                                                                                                                                                                           | エンドポイント                                                                          | 実装レイヤー                                                    | 実装状況 |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- | --------------------------------------------------------------- | -------- |
| **スクリーニング**         | PER、PBR、ROE、配当利回り等の財務指標で銘柄絞り込み<br>株価、出来高、時価総額での条件設定<br>複数条件のAND/OR組み合わせ<br>ソート機能(昇順/降順)<br>プリセット条件の提供(割安株、高配当株、成長株等) | `POST /api/screening/execute`<br>`GET /api/screening/presets`                           | API層<br>サービス層<br>データアクセス層<br>プレゼンテーション層 | 🚧 未実装 |
| **スクリーニング結果保存** | 絞込結果の保存と再利用<br>条件セットの名前付け保存<br>保存済み条件の一覧表示・削除<br>お気に入り条件の管理                                                                                           | `POST /api/screening/save`<br>`GET /api/screening/list`<br>`DELETE /api/screening/{id}` | API層<br>サービス層<br>データアクセス層                         | 🚧 未実装 |
| **データエクスポート**     | CSV/Excel形式でのエクスポート<br>選択列のカスタマイズ<br>エンコーディング選択(UTF-8/Shift-JIS)                                                                                                       | `GET /api/screening/{id}/export`                                                        | API層<br>サービス層                                             | 🚧 未実装 |

### 5. バックテスト(簡易)

| 機能                 | できること                                                                                                                                                                                  | エンドポイント                                                                        | 実装レイヤー                                                                      | 実装状況 |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- | -------- |
| **バックテスト実行** | 期間、初期資金、売買ルールを設定し過去データで戦略検証<br>シンプル移動平均クロス戦略の実装<br>買いエントリー/エグジット条件の設定<br>手数料・スリッページの考慮<br>複数銘柄での並列実行対応 | `POST /api/backtest/start`<br>`GET /api/backtest/{id}/status`                         | API層<br>サービス層<br>データアクセス層<br>プレゼンテーション層                   | 🚧 未実装 |
| **結果可視化**       | 資産曲線、取引ログ、パフォーマンス指標の表示<br>総収益率、シャープレシオ、最大ドローダウンの算出<br>売買タイミングの表示(チャート上)<br>勝率、平均利益/損失の集計                           | `GET /api/backtest/{id}/result`<br>`GET /api/backtest/{id}/trades`                    | API層<br>サービス層<br>プレゼンテーション層<br>フロントエンド: Lightweight Charts | 🚧 未実装 |
| **ジョブ管理**       | バックテスト実行状況の監視<br>実行中ジョブの一覧表示<br>進捗率のリアルタイム更新<br>ジョブのキャンセル機能<br>実行履歴の保存と再確認                                                        | `GET /api/backtest/jobs`<br>`DELETE /api/backtest/{id}/cancel`<br>WebSocket: 進捗配信 | API層<br>サービス層<br>WebSocket: Starlette WebSocket                             | 🚧 未実装 |

### 6. ユーザー設定と認証

| 機能                 | できること                                                                                                                                     | エンドポイント                                                                                             | 実装レイヤー                                                                                                  | 実装状況 |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | -------- |
| **認証**             | JWT認証方式を採用<br>ログイン/ログアウト機能<br>新規ユーザー登録<br>パスワードハッシュ化(bcrypt)<br>トークンリフレッシュ機能<br>セッション管理 | `POST /api/auth/login`<br>`POST /api/auth/logout`<br>`POST /api/auth/register`<br>`POST /api/auth/refresh` | API層<br>サービス層<br>データアクセス層<br>プレゼンテーション層<br>ライブラリ: PyJWT, passlib                 | 🚧 未実装 |
| **プロフィール管理** | ユーザープロフィール編集<br>メールアドレス、表示名の変更<br>パスワード変更<br>アバター画像のアップロード(オプション)                           | `GET /api/user/profile`<br>`PUT /api/user/profile`<br>`PUT /api/user/password`                             | API層<br>サービス層<br>データアクセス層<br>プレゼンテーション層                                               | 🚧 未実装 |
| **通知設定**         | 通知のオン/オフ設定<br>株価アラート(目標価格到達時)<br>バッチ処理完了通知<br>バックテスト完了通知<br>メール通知/ブラウザ通知の切り替え         | `GET /api/user/notification-settings`<br>`PUT /api/user/notification-settings`<br>`POST /api/user/alerts`  | API層<br>サービス層<br>データアクセス層<br>プレゼンテーション層                                               | 🚧 未実装 |
| **表示設定**         | テーマ切替(ライト/ダーク)<br>言語切替(日本語/英語)<br>チャート表示設定<br>タイムゾーン設定<br>ブラウザローカルストレージと同期                 | `GET /api/user/display-settings`<br>`PUT /api/user/display-settings`                                       | API層<br>サービス層<br>データアクセス層<br>プレゼンテーション層<br>フロントエンド: CSS Variables/LocalStorage | 🚧 未実装 |

### 共通機能(監視・管理)

| 機能                 | できること                                                                                                                                      | エンドポイント                                                        | 実装レイヤー                                                                                        | 実装状況   |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ---------- |
| **ヘルスチェック**   | システム稼働状態、DB接続、外部API接続の確認                                                                                                     | `GET /health`                                                         | API層<br>サービス層                                                                                 | ✅ 実装済み |
| **バッチ履歴管理**   | バッチ処理の実行履歴、成功/失敗の記録<br>実行日時、対象銘柄数、取得件数、エラー詳細<br>履歴のフィルタリング・ソート機能<br>ページネーション対応 | `GET /api/v1/batch/history`<br>`DELETE /api/v1/batch/cancel/{job_id}` | API層<br>サービス層<br>データアクセス層                                                             | ✅ 実装済み |
| **リアルタイム進捗** | WebSocketによる進捗配信、ETA表示                                                                                                                | WebSocket                                                             | プレゼンテーション層: Starlette WebSocket<br>フロントエンド: `app/static/script.js` (WebSocket API) | 🚧 未実装   |
