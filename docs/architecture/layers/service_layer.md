...
- [サービス層 仕様書](#サービス層-仕様書)
  ...
  - [1. 概要](#1-概要)
    ...
    - [設計原則](#設計原則)
  - [2. 構成](#2-構成)
    - [ディレクトリ構造（ドメイン駆動設計）](#ディレクトリ構造ドメイン駆動設計)
    - [依存関係（目的別図：設計者向け・開発者向け）](#依存関係目的別図設計者向け開発者向け)
      - [設計者向け（高レベル図）](#設計者向け高レベル図)
      - [開発者向け（実装詳細の参照）](#開発者向け実装詳細の参照)
      - [図の読み方の補足（凡例）](#図の読み方の補足凡例)
    - [レイヤー間の通信（非同期パターン）](#レイヤー間の通信非同期パターン)
  - [3. サービスモジュール一覧（ドメイン駆動設計）](#3-サービスモジュール一覧ドメイン駆動設計)
    - [3.1 共通基盤層 (Core)](#31-共通基盤層-core)
    - [3.2 市場データドメイン (Market Data)](#32-市場データドメイン-market-data)
      - [3.2.1 株価データサブドメイン (Stock Price)](#321-株価データサブドメイン-stock-price)
      - [3.2.2 銘柄マスタサブドメイン (Stock Master / JPX)](#322-銘柄マスタサブドメイン-stock-master--jpx)
    ...
    - [3.5 ユーザー管理ドメイン (User)](#35-ユーザー管理ドメイン-user)
      - [3.5.1 認証サブドメイン (Auth)](#351-認証サブドメイン-auth)
      - [3.5.2 プロフィールサブドメイン (Profile)](#352-プロフィールサブドメイン-profile)
      - [3.5.3 設定サブドメイン (Settings)](#353-設定サブドメイン-settings)
      - [3.5.4 ポートフォリオサブドメイン (Portfolio)](#354-ポートフォリオサブドメイン-portfolio)
    - [3.6 通知ドメイン (Notification)](#36-通知ドメイン-notification)
    - [3.7 システム監視ドメイン (Monitoring)](#37-システム監視ドメイン-monitoring)
  - [4. クラス図（ドメイン駆動設計）](#4-クラス図ドメイン駆動設計)
    - [4.1 共通基盤層 (Core)](#41-共通基盤層-core)
    - [4.2 市場データドメイン - 株価データ・銘柄マスタ](#42-市場データドメイン---株価データ銘柄マスタ)
    - [4.3 バッチ処理基盤モジュール（汎用設計）](#43-バッチ処理基盤モジュール汎用設計)
  - [5. シーケンス図（ドメイン駆動設計）](#5-シーケンス図ドメイン駆動設計)
    - [5.1 株価データ取得・保存フロー（共通基盤の活用）](#51-株価データ取得保存フロー共通基盤の活用)
    - [5.2 銘柄マスタ更新フロー（JPX）](#52-銘柄マスタ更新フローjpx)
    - [5.3 汎用バッチ処理フロー（株価・銘柄マスタ共通）](#53-汎用バッチ処理フロー株価銘柄マスタ共通)
    - [5.4 ファンダメンタルデータ取得フロー](#54-ファンダメンタルデータ取得フロー)
  - [6. 主要サービス詳細](#6-主要サービス詳細)
    - [6.1 共通基盤層の設計パターン](#61-共通基盤層の設計パターン)
      - [Strategy パターン (BaseFetcher/BaseSaver)](#strategy-パターン-basefetcherbasesaver)
      - [Template Method パターン](#template-method-パターン)
    - [6.2 依存性注入パターン（ドメイン別）](#62-依存性注入パターンドメイン別)
      - [市場データドメイン](#市場データドメイン)
      - [バッチ処理基盤](#バッチ処理基盤)
  - [7. 型定義とPydantic統合](#7-型定義とpydantic統合)
  - [8. ドメイン駆動設計のメリット](#8-ドメイン駆動設計のメリット)
    - [8.1 従来設計との比較](#81-従来設計との比較)
    - [8.2 実装時の注意点](#82-実装時の注意点)
  - [関連ドキュメント](#関連ドキュメント)
