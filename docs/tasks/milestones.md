...
- [マイルストーン一覧](#マイルストーン一覧)
  ...
  - [3. マイルストーン詳細](#3-マイルストーン詳細)
    ...
    - [v0.4.0 - 最小Web画面（Flask製、一時的）](#v040---最小web画面flask製一時的)
...
gantt
    ...
    v0.4.0 最小Web画面(Flask)      :v04, after v03, 3d
...
| v0.4.0 最小Web画面(Flask) | プレゼンテーション(Flask)                    | なし（FastAPI実行用の最小UI）                     | 一時的な実装（後で削除）         |
...
### v0.4.0 - 最小Web画面（Flask製、一時的）

**期間:** 3日
**目標:** FastAPIを簡単に実行するための最小Web画面をFlaskで作成

**注意:** このマイルストーンで作成する機能は一時的なもので、v0.4.1以降の本格的なUIが完成した後に削除されます。

**成果物:**
- [ ] Flaskアプリケーションの基本設定
- [ ] シンプルなダッシュボード画面
  - [ ] FastAPIエンドポイント呼び出しボタン
  - [ ] バッチ処理開始ボタン
  - [ ] ステータス表示エリア
- [ ] FastAPIとの通信機能
  - [ ] HTTPリクエスト送信
...
**対象レイヤー:**
- プレゼンテーション層: Flaskアプリケーション・テンプレート
- なし: FastAPIとは独立した簡易アプリ

**特記事項:**
- この実装は開発初期の動作確認用で、保守や拡張は前提としない
- Flaskのベーシックな機能のみ使用
...
目的: Flask製最小Web画面（FastAPI実行用、一時的）
主対象レイヤー: プレゼンテーション(Flask)
完了条件: Web画面からFastAPIを実行・ステータス表示
依存: v0.3.0
優先度: High
特記: 開発初期の動作確認用、v0.4.1完成後削除予定
